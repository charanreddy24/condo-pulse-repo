...
const ImportantContacts = () => {
  ...
  const [quillContent, setQuillContent] = useState(
    '<p>No contacts. Please inform Supervisor to Update the contact list</p>',
  );
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const res = await fetch('/api/infoPages/getImportantContacts');
        if (!res.ok) {
  ...
  const handleSaveButton = async (e) => {
    ...
    setIsEditMode(false);
    try {
      const res = await fetch('/api/infoPages/importantContacts', {
        method: 'PUT',
        body: JSON.stringify({
